...
class Network(object):
    ...
    def __init__(self, name, hyperparameters=dict()):
        ...
        self.update_func             = self.__build_update_func()
    ...
    def __load_params(self, hyperparameters):
        ...
        # Glorot/Bengio weight initialization, additonal factor of 1/2
        def initialize_layer(n_in, n_out):
            rng = np.random.RandomState()
            W_values = np.asarray(
                rng.uniform(
                    low=-np.sqrt(6. / (n_in + n_out))/2,
                    high=np.sqrt(6. / (n_in + n_out))/2,
                    size=(n_in, n_out)
                ),
    ...
    def __build_weakly_clamped_phase(self):
        ...
        weights_dot           = T.grad( E_mean_weakly_clamped, self.weights, consider_constant=layers_weakly_clamped)
        biases_dot            = T.grad( E_mean_weakly_clamped, self.biases,  consider_constant=layers_weakly_clamped)

        weakly_clamped_phase = theano.function(
            inputs=[n_iterations, epsilon, beta],
            outputs=biases_dot + weights_dot
        )

        return weakly_clamped_phase

    def __build_update_func(self):

        db = [T.fvector("db"+str(r+1)) for r in range(len(self.weights))]
        dw = [T.fmatrix("dw"+str(r+1)) for r in range(len(self.weights))]
        alphas = [T.fscalar("alpha_W"+str(r+1)) for r in range(len(self.weights))]

        updates = dict()

        biases_new  = [b - alpha * dot for b,alpha,dot in zip(self.biases[1:],alphas, db)]
        weights_new = [W - alpha * dot for W,alpha,dot in zip(self.weights,   alphas, dw)]
